...
* Use `Add-TfsPendingChange` to `-Add` or `-Edit` files, based on if they are already added to the workspace. 
  ...
  * If `-FullCommit` is not used, it will only attempt to stage files that have been modified or added in the last 48 hours. Because this script runs nightly, this will significantly decrease the run time against large workspaces and still allow for a single nightly failure before potentially missing something.
...
$tfsServer = Get-TfsServer -Name http://coderepo.burmat.co:8080/tfs/Projects"
$vcServer = $tfsServer.GetService([Microsoft.TeamFoundation.VersionControl.Client.VersionControlSer
ver])
$vcProject = $vcServer.GetTeamProject("PROJECT1")
$workspace = $vcServer.GetWorkspace("TFS-PROJECT1", $env:USERNAME)
if ($workspace -ne $null) { Write-Host "FOUND WORKSPACE"; }
$vcServer.DeleteWorkspace("TFS-PROJECT1", $env:USERNAME)
True
